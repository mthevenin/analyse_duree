cox.zphold <- function(fit, transform='km', global=TRUE) {
  call <- match.call()
  ...
  if (is.R()) class(temp) <- "cox.zphold"
  else oldClass(temp) <- "cox.zphold"
  temp
}

"[.cox.zphold" <- function(x, ..., drop=FALSE) {
  i <- ..1
...
print.cox.zphold <- function(x, digits = max(options()$digits - 4, 3),...)
