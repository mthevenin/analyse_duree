...
Le test est exécuté avec la fonction **`coxzph`**.....mais.

Depuis la  v3 de la librairie Survival, il s'agit non pas pas d'une nouvelle version du test, mais de la version d'origine qui est exécutée. Tous les autres logiciels utilisent une version *simplifiée* de ce test. Il y a peu de conséquences en l'absence d'évènements simultanés, situation plutôt fréquente en sciences sociales. Avec le test *exact* on observe des violations quasi systématiques de l'hypothèse en présence d'un nombre importants d'évènements observés simulatnéments.  Je conseille donc, même si une investigation méthodologique plus complète doit être faite, d'utiliser le test de la version précédente. Je donne une procédure plutôt simple d'exécuter ce test dans le chapitre *programmation*, en récupérant directement la fonction. Seul le nom de la fonction a été modifié (**`coxzphold`**).
:::
...
L'intéraction $surgery\times t$ est ici significative ($p<0.05$). On retrouve donc le résultat du test sur les résidus de Schoenfeld.  

**Intreprétation**:  

* Le paramètre (logHR) pour surgery donne le risque ratio au début de l'exposition au risque ($t=0+\epsilon$): le risque de décéder en début d'observation, le premier jour après l'inscription dans le registre, est $(e^{-1.75}-1)\times 100 = -82\%$ plus faible pour les personnes qui ont eu un pontage.
...
| $t$         | Calcul                        | Risk ratio   | 
| ----------- | ----------------------------- | ------------ |
| $0+\epsilon$| $e^{-1.75 + 0.002\times 0}$   |  $0.28$      | 
| 1           | $e^{-1.75 + 0.002\times 1}$   |  $0.281$     | 
| 2           | $e^{-1.75 + 0.002\times 2}$   |  $0.282$     |  
| .           | .                             |              |
| .           | .                             |              | 
| 10          | $e^{-1.75 + 0.002\times 10}$  |  $0.286$     | 
| 100         | $e^{-1.75 + 0.002\times 100}$ |  $0.34$      | 
| 365         | $e^{-1.75 + 0.002\times 365}$ |  $0.58$      | 


Attention, les calculs du tableau précédent ont été donnés à titre d'exemple. Les intervalles de confiance des rapports de risque s'avère nécessaires compte tenu de la diminution de la population restante exposée au risque.
