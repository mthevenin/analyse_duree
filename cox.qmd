...
Une maj va être réalisée en novembre, pour expliquer le plus clairement possible la différences entre le test simplifié (Stata, Sas, R jusqu'à la v3) et le test exact (R à partir de la V3).  

Résumé rapide: le test consiste à regarder la corrélation entre des résidus obtenus directement avec la fonction de score de la vraisemblanced partielle de Cox.  

- Test simplifié: simple corrélation obtenu par une régression linéaire par les mondres carrés ordinaires. Pour chaque covariable, on utilise la variance de son estimateur obtenu par le modèle.
- Test exact:  corrélation obtenu par une régression linéaire par les mondres carrés généralisés. Pour chaque covariable, on calcule la variance du résidu après avoir calculé la matrice d'information de fisher (seulement les termes sur la diagonale). Le modèle linéaire généralisés vont prendre en compte l'autocorrélation des résidus inhérente à une analyse sur données temporelles.
::: 
...
$$rs_{ik}=X_{ik}- \sum_{j\in R_i}X_{jk}\frac{e^{X_{j}^{'}b}}{\sum_{j\in R_i}e^{X_{j}^{'}b}}= X_{ik} - E(X_k)$$
...
On retrouve donc un résultat proche de celui obtenu à partir du test simplifié sur les résidus de Schoenfeld pour la variable *surgery*. Avec $f(t)=t$, il a le mérite de pouvoir être interprété directement. Ce qui ne veut pas dire qu'il s'agit de la meilleure solution.    
...
Le test de **Grambsch-Therneau** est donc aussi un test qui donne des éléments d'informations sur de possibles effets de sélection, ou plus généralement sur la présence de biais d'omission dans le modèle.
