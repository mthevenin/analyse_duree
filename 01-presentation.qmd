...
3. ***Des variables explicatives*** ou ***covariables***
   ...
   - Dynamiques (TVC: *Time varying covariates*): 
     ...
     - Pour les modèles et à l'exception d'une méthode traité pluse loin, à savoir le modèle semi-paramétrique de Cox, en présence de données censurée, la durée ou une transformation de celle-ci est une variable dynamique qui est également introduite comme variable indépendante pour assurer le bon ajustement des données. L'introduction de la durée ou une transformation de celle-ci comme variable dépendante ne peut se faire qu'en absence d'observation censurée, en particulier à droite.
