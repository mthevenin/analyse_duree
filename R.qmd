...
```{r filename=" installation survival 2.44-1 (30 mars 2019)", eval=FALSE}
...
Il serait également bienvenu de préciser dans une analyse la version du test qui a été exécutée.
:::
...
Estimé simplement à partir d'une OLS (normalement préféré une GLS pour tenir compte de l'autocorrélation des erreurs)
...
On est trés proche du test effectué avec la v2 de `survival`. 
:::
